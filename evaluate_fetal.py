...
from scipy.ndimage import distance_transform_edt, binary_fill_holes # Added binary_fill_holes
...
FETAL_DATA_DIR = "./fetal_data/training_set/training_set"
...
class FetalUltrasoundDataset(Dataset):
    def __init__(self, data_dir, image_size=1024):
        ...
        self.image_files = sorted([f for f in all_files if f.endswith('.png') and 'Annotation' not in f])
...
def evaluate_fetal():
    ...
    dataset = FetalUltrasoundDataset(data_dir=FETAL_DATA_DIR)
    sampler = DistributedSampler(dataset, shuffle=False)
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler, num_workers=2, pin_memory=True)

    if local_rank == 0:
        print(f"Total Fetal images: {len(dataset)}")

    if local_rank == 0: print("\n--- Testing Baseline MedSAM on Fetal ---")
    ...
    if local_rank == 0: print(f"\n--- Testing PINN (Trained on Breast) on Fetal ---")
    ...
    if local_rank == 0:
        print("\n==========================================")
        print("FETAL DATASET GENERALIZATION SCORES")
        print("==========================================")

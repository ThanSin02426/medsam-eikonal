...
IMAGE_DIR = "./medsam1/polyp_data/Kvasir-SEG/Kvasir-SEG/images"
MASK_DIR = "./medsam1/polyp_data/Kvasir-SEG/Kvasir-SEG/masks"
...
class PolypDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # In Kvasir-SEG, masks usually have the exact same filename
        ...
        # Binarize and fill any accidental holes in the polyp mask
        mask_bin = (mask > 127)
...
def evaluate_polyp():
    ...
    dataset = PolypDataset(image_dir=IMAGE_DIR, mask_dir=MASK_DIR)
    sampler = DistributedSampler(dataset, shuffle=False)
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler, num_workers=2, pin_memory=True)

    if local_rank == 0:
        print(f"Total Polyp images: {len(dataset)}")

    if local_rank == 0: print("\n--- Testing Baseline MedSAM on Polyps ---")
    ...
    if local_rank == 0: print(f"\n--- Testing PINN (Trained on Breast) on Polyps ---")
    ...
    if local_rank == 0:
        print("\n==========================================")
        print("POLYP DATASET GENERALIZATION SCORES")
        print("==========================================")
